      ...
      >
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <input
            ...
            onKeyDown={onKeyDown}
            placeholder="Search titles, notes and code…"
            role="combobox"
            aria-expanded={hits.length > 0}
            aria-controls="lk-search-results"
            aria-activedescendant={hits[active] ? `lk-hit-${hits[active].id}` : undefined}
        ...
        <div ref={listRef} id="lk-search-results" role="listbox" className="max-h-[60vh] overflow-y-auto p-1.5">
          {tooShort
            ...
            : isError
              ...
              : hits.length === 0
                ? message(isFetching ? "Searching…" : `Nothing matches "${q.trim()}".`)
                : hits.map((hit, i) => (
                    <button
                      key={`${hit.kind}-${hit.id}`}
                      id={`lk-hit-${hit.id}`}
                      ...
                      onClick={() => go(hit)}
                      className={`lk-subject flex w-full items-start gap-3 rounded-md px-2.5 py-2 text-left ${
                        i === active ? "bg-muted" : ""
                      }`}
                      style={{ "--c": hit.color ?? FALLBACK } as React.CSSProperties}
                    >
                      <span className="lk-swatch mt-1.5 flex-none" />
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center gap-2">
                          <span
                            className={`truncate text-sm ${hit.isCompleted ? "text-muted-foreground" : ""}`}
                          >
                            <Highlight text={hit.title} terms={terms} />
                          </span>
                          <span className="lk-tag flex-none">{hit.kind}</span>
                        </span>
                        {hit.path.length > 0 && (
